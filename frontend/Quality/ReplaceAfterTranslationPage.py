...
class ReplaceAfterTranslationPage(QWidget, Base):
    ...
    def __init__(self, text: str, window: FluentWindow) -> None:
        ...
        # 默认配置
        self.default = {
            "replace_after_translation_enable": True,
            "replace_after_translation_data" : [
                {
                    "src": "…。",
                    "dst": "…"
                },
    ...
    def add_widget_head(self, parent: QLayout, config: dict, window: FluentWindow) -> None:
        ...
        def init(widget: SwitchButtonCard) -> None:
            widget.set_checked(config.get("replace_after_translation_enable"))

        def checked_changed(widget: SwitchButtonCard, checked: bool) -> None:
            config = self.load_config()
            config["replace_after_translation_enable"] = checked
    ...
    def add_widget_body(self, parent: QLayout, config: dict, window: FluentWindow) -> None:
        ...
        self.table.setColumnCount(len(ReplaceAfterTranslationPage.KEYS))
    ...
    def add_command_bar_action_import(self, parent: CommandBarCard, config: dict, window: FluentWindow) -> None:
        ...
        def triggered() -> None:
            ...
            # 从文件加载数据
            data = TableHelper.load_from_file(path, ReplaceAfterTranslationPage.KEYS)

            # 读取配置文件
            config = self.load_config()
            config["replace_after_translation_data"].extend(data)

            # 向表格更新数据
            TableHelper.update_to_table(self.table, config["replace_after_translation_data"], ReplaceAfterTranslationPage.KEYS)

            # 从表格加载数据（去重后）
            config["replace_after_translation_data"] = TableHelper.load_from_table(self.table, ReplaceAfterTranslationPage.KEYS)
    ...
    def add_command_bar_action_reset(self, parent: CommandBarCard, config: dict, window: FluentWindow) -> None:
        ...
        def triggered() -> None:
            ...
            # 加载默认设置
            config["replace_after_translation_data"] = self.default.get("replace_after_translation_data")
